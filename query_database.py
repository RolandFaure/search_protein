...
def query_faiss_database(original_fasta, database_folder, query_sequences, query_names, cutoff=0.2, num_gpus=1, gpus_available=True, subdatabase_size=10000000, max_workers=4):
    ...
    # embeddings_cache_file = os.path.join(database_folder, "/pasteur/appa/scratch/rfaure/queries/papilloma/query_embeddings_cache.npy")
    # try:
    #     # np.save(embeddings_cache_file, query_embeddings)
    #     # print(f"Embeddings saved to {embeddings_cache_file}")
        
    #     # Load embeddings back from disk
    #     query_embeddings = np.load(embeddings_cache_file)
    #     print(f"Embeddings loaded from {embeddings_cache_file}")
    # except Exception as e:
    #     print(f"Warning: Could not save/load embeddings cache: {e}")
...
if __name__ == "__main__":
    ...
    with open(fasta_output, "w") as fasta_file:
        for name, seq in all_results:
            if '>' in seq:
                print("WHWUHIA ", seq)
            if len(name.split()[0].split("_")) < 4 : #that's becausde the human and nonhuman db are not exactly formatted the same way
