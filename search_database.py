...
if __name__ == "__main__":
    ...
    parser.add_argument("--embeddings", required=True, help="Path to the embeddings file (.npy)")
    ...
    query_embeddings = np.load(args.embeddings)
    print(f"Loaded embeddings with shape {query_embeddings.shape}")

    # Load query names
    names_file = args.embeddings.replace('.npy', '.names.txt')
    query_names = []
